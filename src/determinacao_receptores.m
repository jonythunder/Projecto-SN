...
for pr_line=1:size(input_raw,1)
    ...
    base_pos = 0;
    [xyz,sat_pos,clock_bias]=calc_position(input_hui,eph_aux,input_eph,TOW,WN,pr_filtered);
...
title("Erro de posição UB2")
figure
plot(1:pr_line,PDOP_1)
hold on
plot(1:pr_line,HDOP_1)
title("PDOP e HDOP UB1")
legend('PDOP','HDOP')
figure
plot(1:pr_line,PDOP_2)
hold on
plot(1:pr_line,HDOP_2)
title("PDOP e HDOP UB2")
legend('PDOP','HDOP')
...
fprintf("O erro médio para UB1 é:\n")
disp(sum(error_history/size(input_raw,1)));

fprintf("A posição média para UB2 é:\n")
disp(avg_xyz2)
disp(avg_llh_RF2);

fprintf("O erro médio para UB2 é:\n")
disp(sum(error_history2/size(input_raw2,1)));